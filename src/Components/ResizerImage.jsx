...
export default function ResizerImage() {
  ...
  const applyPreset = (preset) => {
    setTargetWidth(presets[preset].width);
    setTargetHeight(presets[preset].height);
  ...
  const updateCrop = (e) => {
    ...
    setCropArea({
      ...
      width: Math.min(width, originalDimensions.width - dragStart.x),
      height: Math.min(height, originalDimensions.height - dragStart.y),
    });
  };
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4">
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  placeholder="Width"
                  className="bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white placeholder-gray-400 focus:outline-none focus:border-purple-500"
                />
                <input
                  type="number"
                  value={targetHeight}
            ...
            >
              {cropMode ? "✂️ Crop Mode Active" : "✂️ Enable Crop"}
            </button>

            {/* Effects Toggle */}
            <button
              onClick={() => setShowEffects(!showEffects)}
              className="w-full py-3 rounded-xl bg-white/10 text-gray-300 hover:bg-white/20 transition-all"
            >
              ...
              <div className="flex items-center justify-center gap-6 text-sm">
                ...
                {showEffects &&
                  (brightness !== 100 ||
                    contrast !== 100 ||
                    saturation !== 100) && (
                    <span className="text-pink-400">✨ Enhanced</span>
                  )}
              </div>
            </div>
          )}
